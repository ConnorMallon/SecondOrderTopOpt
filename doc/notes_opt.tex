...
\end{equation}
We can prove (see e.g. \cite{NocedalWright2006}) that choosing $\eta_k = O\left(\left\Vert \frac{d\hat J}{dp}(p_k) \right\Vert\right)$ leads to linear convergence for the iterations:
\begin{equation}
...
Since the matrix is not necessarily positive definite, we also terminate if a direction of negative curvature is found \cite{NocedalWright2006}.
\begin{equation}
\tilde{p}_k^T H(p_k)\tilde{p}_k < 0.
\end{equation}
An adaptive trust region is also used.