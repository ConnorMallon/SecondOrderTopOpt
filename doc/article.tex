...
% (2) add a space before each % so flags don’t get glued onto the filename
\newcommand{\triggerexport}[2][1]{%
...
% (3) call \triggerexport with the actual path, not \import@path
\newcommand{\incfig}[2][1]{%
  \triggerexport[#1]{./figures/#2}%
  \def\svgwidth{#1\linewidth*\real{1}}%
  \large% ← change this to \large, \Huge, whatever you like
...
% 	\centering
%     \incfig[1.0]{graph}
%   \caption{There are far more connections to complete the computational graph - i.e. we need to include arrows from $\dot{u}$ to $\dot{\delta u}$}
% 	\label{fig:topology-convergence}
%     % see fig_convergence.jl and fig_initial_topology.jl
% \end{figure}
